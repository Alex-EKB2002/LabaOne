...
public class ImportDiametr : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (IsTrue && textt.text != "" && Convert.ToInt32(textt.text) >= 5 && Convert.ToInt32(textt.text) <= 25)
        {
            ...
            lenZam.text = (Convert.ToInt32(textt.text) * 5).ToString() + " ìì";
            a0Zam.text = string.Format("{0:0.##}", (Convert.ToInt32(textt.text) * (Convert.ToInt32(textt.text) * Math.PI)) / 4) + " êâ.ìì";
            v0Zam.text = string.Format("{0:0.##}", Convert.ToDouble(a0.text.Remove(a0.text.Length - 6, 6)) * Convert.ToInt32(len.text.Remove(len.text.Length - 3, 3)) + " êóá.ìì");
            diamZam.text = Convert.ToInt32(textt.text).ToString() + " ìì";
        }
        else
        {
